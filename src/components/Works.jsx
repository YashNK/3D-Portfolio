...
import { github, xplrlogo } from '../assets'
...
const ProjectCard = ({index, name, description, tags, image, source_code_link, xplr_page_link, Page_Logo}) => {
  return(
    <motion.div variants={fadeIn("up","spring", index * 0.5, 0.75)}>
      ...
      className="bg-tertiary p-5 rounded-2x1 w-full xs:w-[360px] sm:w-[360px]"
      >
        <div className='relative w-full h-[230px]'>
          <img src={image}
          alt={name}
          className='w-full h-fullobject-cover rounded-2x1' />
          <div className='absolute inset-0 flex justify-end m-3 card-img_hover'>
            <div onClick={() => window.open(source_code_link, "-blank")}
            className='black-gradient mr-1.5 w-10 h-10 rounded-full flex justify-center items-center cursor-pointer'>
              <img src={github} 
              alt="github"
